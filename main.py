...
def voice_command_listener():
    ...
    while True:
        ...
        if recognizer.AcceptWaveform(data):
            ...
            elif any(word in command for word in ["situation", "sitting", "position", "reposition", "calibrate", "celebrate", "recal", "callibrate"]):
                recalibrated = False
                calibration_start = time.time()
                print("🛠 Voice Command: Recalibration triggered")

            # Zoom controls
            elif any(word in command for word in ["zoom in", "soon in", "room in", "zoomin", "zooming"]):
                zoom_control("in")
            elif any(word in command for word in ["zoom out", "soon out", "room out", "zoomout"]):
                zoom_control("out")
            elif any(word in command for word in ["reset zoom", "default zoom", "original", "reset", "default view", "reset view"]):
                zoom_control("reset")

            # Scrolling commands (only active in voice-only mode)
            elif voice_only_mode and scroll_enabled:
                if any(word in command for word in ["up", "cup", "hub", "pop"]):
                    scroll_once("up")
                elif any(word in command for word in ["down", "drown", "don't", "don"]):
                    scroll_once("down")
                elif any(word in command for word in ["left", "let", "let's", "life"]):
                    scroll_once("left")
                elif any(word in command for word in ["right", "night", "light", "fight"]):
                    scroll_once("right")
